...
#
#     gt_boxes_area = ((gt_boxes[:,2] - gt_boxes[:,0] + 1) *
#                 (gt_boxes[:,3] - gt_boxes[:,1] + 1)).view(1, K)   #(1, K)
#
#     anchors_area = ((anchors[:,2] - anchors[:,0] + 1) *
#                 (anchors[:,3] - anchors[:,1] + 1)).view(N, 1)
#
...
#     # query_boxes = gt_boxes.view(1, K, 4).expand(N, K, 4)
#
#     iw = (torch.min(anchors[:,2], gt_boxes[:,2]) - torch.max(anchors[:,0], gt_boxes[:,0]) + 1)
#     iw[iw < 0] = 0
#     # w = max(0, xmax - xmin)
#
#     ih = (torch.min(anchors[:,3], gt_boxes[:,3]) - torch.max(anchors[:,1], gt_boxes[:,1]) + 1)
#     ih[ih < 0] = 0
#
#
